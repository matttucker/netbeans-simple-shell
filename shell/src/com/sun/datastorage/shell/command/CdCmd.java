/**
 * The contents of this file are subject to the terms of the Common Development
 * and Distribution License (the License). You may not use this file except in
 * compliance with the License.
 *
 * You can obtain a copy of the License at http://www.netbeans.org/cddl.html
 * or http://www.netbeans.org/cddl.txt.
 *
 * When distributing Covered Code, include this CDDL Header Notice in each file
 * and include the License file at http://www.netbeans.org/cddl.txt.
 * If applicable, add the following below the CDDL Header, with the fields
 * enclosed by brackets [] replaced by your own identifying information:
 * "Portions Copyrighted [year] [name of copyright owner]"
 *
 * The Original  Software is Simple Shell. The Initial Developer of the Original
 * Software is Sun Microsystems, Inc.
 *
 * Matt Tucker, Sun Microsystems
 *
 */
package com.sun.datastorage.shell.command;

import com.sun.datastorage.shell.CmdLineParser;
import com.sun.datastorage.shell.Command;
import com.sun.datastorage.shell.Commander;
import com.sun.datastorage.shell.ShellEditorDocument;
import com.sun.datastorage.shell.completion.FileCompletionFilter;
import java.io.IOException;
import org.netbeans.spi.editor.completion.CompletionResultSet;

class CdCmd extends Command {

    public CdCmd(Commander cmdr) {
        super(cmdr, "cd", "com.sun.datastorage.shell.command.cd", "nbdocs://com.sun.datastorage.shell/com/sun/datastorage/shell/docs/shell-command-cd.html");
    }

    @Override
    public void printUsage(String[] args) {
        cmdr.getDoc().getOutWriter().println("Usage: cd <dir>");
    }

    @Override
    public void execute(String[] args) throws Exception {
        super.execute(args);
        if (args != null) {

            if (otherArgs.length == 1) {
                try {
                    cmdr.getDoc().getPathManager().setCwd(otherArgs[0]);
                } catch (IOException ex) {
                    cmdr.getDoc().getErrorWriter().println("Directory does not exist.");
                }
            }
        } else {
            try {
                cmdr.getDoc().getPathManager().setCwd(System.getProperty("user.home"));
            } catch (IOException ex) {
                cmdr.getDoc().getErrorWriter().println("Directory does not exist.");
            }
        }
    }

    @Override
    public void addArgCompletionItems(final CompletionResultSet completionResultSet,
            ShellEditorDocument doc, String filter, int startOffset,
            int caretOffset, String[] cmdArgs, int argNum) {
        if (argNum <= 1) {
            //
            // add file names that match filter
            //
            addFileCompletionItems(completionResultSet, doc, filter, startOffset, caretOffset, FileCompletionFilter.DIRS_ONLY);
        }
    }
}
